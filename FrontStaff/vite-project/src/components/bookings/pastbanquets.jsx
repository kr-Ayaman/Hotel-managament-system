...
function PastBanquetsTable() {
  ...
  useEffect(() => {
    const fetchPastBanquets = async () => {
      try {
        const responseData = await sendRequest(
          "http://localhost:5000/staffportal/past-booking/banquets",
          "GET",
          null,
          {
        ...
        const fetchedBanquets = responseData.result.map((banquet) => ({
          ...
          bookedfrom: banquet.check_in_date.split("T")[0], // Format date
          bookedupto: banquet.check_out_date.split("T")[0], // Format date
          occasion: banquet.occasion,
          phone: banquet.phone_number,
        }));
        setBanquets(fetchedBanquets);
        setFilteredBanquets(fetchedBanquets); // Initially, display all banquets
      } catch (err) {
        console.error(err);
      }
    };
...
export default PastBanquetsTable;
